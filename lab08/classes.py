...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        best_card = None
        if player.hand:
            power_value = player.hand[0].power(opponent_card)
            for i in range(len(player.hand)):
                if player.hand[i].power(opponent_card)>power_value:
                    power_value = player.hand[i].power(opponent_card)
                    power_value_index = player.hand[i]
            best_card = power_value_index
            self.attack += best_card.attack
            self.defense += best_card.defense
            player.hand.remove(power_value_index)
